...
//Función agregada para Desafío M4
...
//Función modificada utilizando la función anterior
function obtenerBalanceMensual(saldo)
{
    let estado = obtenerEstadoBalance(saldo);
    let mensajeBalance = "";
    switch (estado) {
        case -1:
            mensajeBalance = `\nPerdidas: -$${-saldo}.`;
            break;    
        case 0:
            mensajeBalance = `\nLos ingresos son iguales a las ganancias.`;
            break;
        case 1:
            mensajeBalance = `\nGanancias: $${saldo}.`;
            break;
        default:
            mensajeBalance = `Se produjo un error al obtener el balance.`;
            break;
    }
    return mensajeBalance;
}
